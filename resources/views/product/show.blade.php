...
<div class="container my-5">
    <div class="row">
        <div class="col-lg-4 mx-auto card p-4">
            <form action="{{ route('admin.products.index') }}/{{ $product->id }}" method="POST" class="form-group"
                ...
                <textarea name="content" id="content" rows="3"
                    class="form-control mb-3 @error('content') is-invalid @enderror">{{ old('content', optional($product)->content) }}</textarea>
                @error('content')
                <div class="alert alert-danger">{{ $message }}</div>
                @enderror
                ...
                <input type="text" name="weight" id="weight"
                    ...
                    value="{{ old('weight', optional($product)->weight) }}">
                @error('weight')
                <div class="alert alert-danger">{{ $message }}</div>
                @enderror
